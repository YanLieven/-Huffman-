...
void Huffman::comprimir(QString string, QString string2)
{
        ...
        QByteArray line;
        QByteArray linha; // Conterá o texto do arquivo original completo
        while (!file.atEnd()) {
        ...
        codif.resize(codif.size() + lixo);
        codif.resize(codif.size()/8);
        linha.resize(2);
        ...
        newfile.write(linha); // nesta e nas 3 linhas abaixo ocorre a compressão.
        newfile.write(xcv);
        newfile.write(codif);
    ...
    void Huffman::descomprimir(QString string, QString string2)
    {
        QFile fil(string);
